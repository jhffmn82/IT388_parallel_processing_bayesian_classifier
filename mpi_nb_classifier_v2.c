 ...
 *
 * Compile:  mpicc mpi_nb_classifier_v2.c -o mpi_nb_v2 -lm
 * Run:      mpiexec ./mpi_nb_v2 <meta.csv> <labeled.csv> <unlabeled.csv> <output.csv> <k> <num_processes>
 */
...
void cross_validate(int* labeled_data,
    ...
    *total_tn = *total_fp = *total_fn = *total_tp = 0;
    ...
    for (fold = 0; fold < k; fold++) {
        ...
        classify_dataset(labeled_data, test_size, labeled_cols, num_features, num_classes,
                         feature_num_values, feature_offsets, class_values,
        ...
        free(train_idx);
        free(test_idx);
        // V2: removed free(train_data) and free(test_data) — no longer allocated
        free(y_train);
        free(y_test);
        free(pred_train);
        free(pred_test);
